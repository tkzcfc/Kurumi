...
local parserpath = string.format("%s.labelparser", currentpath)
...
function RichLabel:ctor(params)
	params = params or {}
	local fontName 	= params.fontName or "fonts/HYh3gj.ttf"
...
-- @brief 设置自定义元素解析回调,函数需要返回一个node,必须在setString之前设置才会回调
function RichLabel:setCustomSpawnHook(call)
	self.onCustomSpawnHook = call
end

-- @brief 设置元素被创建时通知函数,必须在setString之前设置才会回调
function RichLabel:setEleCreateHook(call)
	self.onEleCreateHook = call
end
...
function RichLabel:debugDraw(level)
	...
	local function table_insertto(dest, src, begin)
	    if begin <= 0 then
	        begin = #dest + 1
	    end
	    local len = #src
...
function RichLabel:charsToNodes_(parsedtable, containerNode)
	...
	for index, params in pairs(parsedtable) do
		...
		for _, node in pairs(nodelist) do
			...
			self:doEleCreateHook(node, params)
...
function  RichLabel:convertColor(xstr)
	...
	local blue = tonumber(string.sub(xstr, -2, -1), 16) 
    local green = tonumber(string.sub(xstr, -4, -3), 16) 
    local red = tonumber(string.sub(xstr, -6, -5), 16)
...
function RichLabel:doCustomSpawn(params)
	if self.onCustomSpawnHook then
		return self.onCustomSpawnHook(params)
	end
end

function RichLabel:doEleCreateHook(node, params)
	if self.onEleCreateHook then
		self.onEleCreateHook(node, params)
	end
end
