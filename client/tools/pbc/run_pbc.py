...
def publishPbc(path, targetDir):
	...
	# 清单文件写入
	manifestFile = "-- Auto Generate by script 'run_pbc.py', Don't try to modify!\n\n"
	manifestFile = manifestFile + "local M = {}\n"
	...
	# err code
	manifestFile = manifestFile + "\n"
	manifestFile = manifestFile + "local code = {}\n"
	for name in allErrCodeDict:
		manifestFile = manifestFile + "code['{0}'] = {1}\n".format(name, allErrCodeDict[name])
	manifestFile = manifestFile + "\n"
	manifestFile = manifestFile + "if cc then\n"
	manifestFile = manifestFile + "    cc.exports.errCode = code\n"
	manifestFile = manifestFile + "else\n"
	manifestFile = manifestFile + "    _G.errCode = code\n"
...
def run():
	...
	publishPbc(current_path, current_path + "./../../kurumi/src/pb/")
